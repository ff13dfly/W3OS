	...
	useEffect(() => {
		...
		props.balance(acc.address,(res)=>{
			if(res===false){
				...
				const min=100,cost=props.cost;
				setBalance(amount.toLocaleString());

				//1.too low balance, stop writing to chain.
				if(amount<min) return setLow(`Less than ${min} unit, can not write to chain`);
				
				//2.cost too much, stop writing to chain.
				if(Math.floor(cost/amount)>2) return setLow(`Your balance is too low to try.`);
	...
	return (
			<Row >
				<Col lg = { 12 } xs = { 12 } className="text-muted">
					Cost : <span className="text-dark">{props.cost}</span> unit, balance : <span className="text-dark">{balance}</span> unit 
				</ Col>
				<Col lg = { 12 } xs = { 12 } className="text-danger">{low}</Col>
				<Col lg = { 12 } xs = { 12 } className="pt-1">
					<p style={cls} className="text-warning">{acc.address}</p>
				</Col>
				
				<Col lg = { 12 } xs = { 12 }>{extend}</ Col>
				...
				<Col lg = { 5 } xs = { 5 } className = "pt-2 text-end" >
					<Button size = "lg" variant="primary" onClick={()=>{self.vertify()}} className={disable}>Sign</Button>
				</Col>
			</Row>
	);
}
